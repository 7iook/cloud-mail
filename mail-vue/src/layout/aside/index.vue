  ...
  <el-scrollbar class="scroll">
    <div>
      ...
      <el-menu :collapse="false" text-color="#fff" active-text-color="#fff" style="margin-top: 10px">
        ...
        <div class="manage-title" v-perm="['all-email:query','user:query','role:query','setting:query','analysis:query','reg-key:query','share:create']">
          <div>{{$t('manage')}}</div>
        </div>
        ...
        <el-menu-item @click="router.push({name: 'share'})" index="share" v-perm="'share:create'"
                      :class="route.meta.name === 'share' ? 'choose-item' : ''">
          <Icon icon="material-symbols:share" width="20" height="20" />
          <span class="menu-name" style="margin-left: 21px">邮件分享</span>
        </el-menu-item>
